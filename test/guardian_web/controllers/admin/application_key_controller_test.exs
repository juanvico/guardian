defmodule GuardianWeb.Admin.ApplicationKeyControllerTest do
  ...
  describe "create application_key" do
    ...
    test "renders errors when data is invalid", %{conn: conn} do
      conn = post conn, Routes.admin_application_key_path(conn, :create), application_key: @invalid_attrs
      assert html_response(conn, 200) =~ "New Application key"
    end
  end
  ...
  describe "edit application_key" do
    ...
    test "renders form for editing chosen application_key", %{conn: conn, application_key: application_key} do
  ...
  describe "update application_key" do
    ...
    test "redirects when data is valid", %{conn: conn, application_key: application_key} do
      conn = put conn, Routes.admin_application_key_path(conn, :update, application_key), application_key: @update_attrs
      assert redirected_to(conn) == Routes.admin_application_key_path(conn, :show, application_key)
    ...
    test "renders errors when data is invalid", %{conn: conn, application_key: application_key} do
      conn = put conn, Routes.admin_application_key_path(conn, :update, application_key), application_key: @invalid_attrs
      assert html_response(conn, 200) =~ "Edit Application key"
    end
  end
